...
class CDataTableTransactionAccepted extends Component{
    constructor(props){
        super(props);
        this.state = {
            accepted : [],
        ...
        if(this.props.data == ''){
            ...
            return(
                <div>
                    ...
                    {this.props.data.map((accepted, i)=>{
                        return(
                            <div className="card bg-white mb-2" key={i}>
                                <div className="card-body">
                                    <div className="row">
                                        <div className="col-md-4 col-6">
                                            <h6 className="text-muted">{accepted.order_id}</h6>
                                            <h5>{accepted.product}</h5>
                                            <p className="text-muted">{accepted.date}</p>
                                        </div>
                                        <div className="col-md-3 col-6">
                                            <h6 className="text-muted">Total Tagihan</h6>
                                            <h5>Rp {this.formatCurrency(accepted.total)}</h5>
                                        </div>
                                        <div className="col-md-3">
                                            <h6 className="text-muted">Status Tagihan</h6>
                                            {/* <h5><span className="badge badge-danger text-white">Belum Dibayarkan</span></h5> */}
                                            {this.getStatus(accepted.status)}
                                        </div>
                                        <div className="col-md-2">
                                            <Link to="/partner/product/detail" className="btn btn-outline-danger rounded-0 btn-block">Lihat Detail</Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default CDataTableTransactionAccepted;